...
const MyAccordion = ({ title, data, selectedItem, setSelectedItem }: { title: string; data: any; selectedItem: string; setSelectedItem: any }) => {
    ...
    return (
        <div>
            <div onClick={() => setIsOpen(!isOpen)} className='group hover:cursor-pointer flex items-center justify-between py-6'>
            ...
            <div className={`py-4 flex flex-wrap gap-3 transition-all duration-1000 ${!isOpen ? "hidden" : "block"}`}>
                {data?.map((item: { id: number; title: string; }) => <div key={item.id} onClick={() => setSelectedItem(item.title)} className={`px-4 py-2 ${selectedItem === item.title ? "bg-gray-700 text-white" : "bg-gray-200 text-black"} hover:bg-gray-300 hover:cursor-pointer rounded-lg font-semibold text-sm`}>{item.title}</div>)}
            </div>
