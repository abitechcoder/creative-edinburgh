...
const MemberDetails = () => {
  ...
  const fetchMember = async () => {
    const { data } = await Axios.get(`/members/${params.id}`);
    setMember(data);
  }
    ...
    <Suspense fallback={<Loading/>}>
      ...
      <section className="bg-gray-50 py-16 relative lg:min-h-screen grid grid-cols-1 lg:grid-cols-2">
        <div className="w-full h-full flex flex-col justify-center">
          ...
          <div>
          <p
            className={`${styles.paragraph} text-secondary mb-2 font-bold leading-relaxed text-xl uppercase`}
          >
            SECTOR
          </p>
          <div className="bg-secondary text-white py-3 px-6 w-fit rounded-full">
            {member?.sector}
          </div>
          </div>
        </div>
