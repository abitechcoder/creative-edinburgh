...
import { FaArrowLeft, FaFacebookF, FaInstagram, FaLinkedin, FaCheck } from "react-icons/fa";
      ...
      <div className="pb-8 px-8">
        <section className="bg-gray-50 pt-16 relative lg:min-h-screen grid grid-cols-1 lg:grid-cols-2">
          <div className="w-full h-full flex flex-col items-start justify-center">
            ...
            <div className="flex flex-col lg:flex-row w-full lg:justify-between items-start lg:items-center gap-8">
              <img src={member?.logo === "" ? Logo : member?.logo} alt={`${member?.name} logo`} className="h-[70px] mt-8" />
        ...
        <section className="lg:py-10 py-2 grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="grid lg:grid-cols-2 grid-cols-1 gap-8 lg:gap-4 py-8">
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Years in Operation</h3>
              <p className="font-bold text-lg mt-2 lg:mt-4">{member?.yearsInOperation} years</p>
            </div>
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Revenue</h3>
              <p className="font-bold text-lg mt-2 lg:mt-4">{member?.revenueBracket}</p>
            </div>
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Disability Inclusion</h3>
              <p className="font-bold text-lg mt-2 lg:mt-4">{member?.disabilityInclusion}</p>
            </div>
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Status</h3>
              <div className="mt-4">
                {member?.registrationStatus === "Registered" ? (<FaCheck size={30} className="text-green-600" />) : (<IoCloseCircleSharp size={30} className="text-red-600" />)}
              </div>
            </div>
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Social Media</h3>
              <div className="flex gap-4 mt-2 lg:mt-4">
                {member?.socialMediaLinks && (
                  <>
                    <Link to={member.socialMediaLinks.facebook}>
                      <FaFacebookF size={30} className="hover:text-blue-700" />
                    </Link>
                    <Link to={member.socialMediaLinks.instagram}>
                      <FaInstagram size={30} className="hover:text-blue-700" />
                    </Link>
                    <Link to={member.socialMediaLinks.linkedin}>
            ...
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Contact Number</h3>
              <p className="font-bold text-lg mt-2 lg:mt-4">{member?.contactDetails}</p>
            </div>
            <div>
              <h3 className="font-black uppercase lg:text-xl text-lg text-secondary">Address</h3>
              <p className="font-bold text-lg mt-2 lg:mt-4">{member?.businessAddress}</p>
            </div>
          </div>
        </section>
      </div>
